...
class CLIPResNetWithAttention(nn.Module):
    ...
    def forward(self, x, injection_func=None):
        def stem(x):
            for conv, bn in [(self.conv1, self.bn1), (self.conv2, self.bn2), (self.conv3, self.bn3)]:
                x = self.relu(bn(conv(x)))
            x = self.avgpool(x)
        ...
        # 【核心修改】：通过 Hook 机制在网络正中间注入 Adapter 特征
...
class CLIPTextContextEncoder(nn.Module):
    ...
    def forward(self, text, context):
        ...
        if N_text == B:
            ...
            # 提取 EOS 特征
            eos_feat = x[torch.arange(x.shape[0]), eos_indx] @ self.text_projection

            # 提取序列特征
            seq_feat = x @ self.text_projection
            return eos_feat, seq_feat
            ...
            # --- 【修改点】：返回全局特征和序列特征 ---
            eos_feat = x[torch.arange(x.shape[0]), eos_indx] @ self.text_projection
            eos_feat = eos_feat.reshape(B, K, self.embed_dim)

            seq_feat = x @ self.text_projection
            seq_feat = seq_feat.reshape(B, K, -1, self.embed_dim)
