...
class CLIPTextEncoder(nn.Module):
    ...
    def build_attention_mask_for_seq_len(self, seq_len):
...
class CLIPTextContextEncoder(nn.Module):
    ...
    # Replace the forward method in the second forward function (the one that takes both text and context)
    def forward(self, text, context):

        x_text = self.token_embedding(text)  # [B*2, 72, C]

        if context.dim() == 3:
            B, N_ctx, C = context.shape
        else:
            B = context.shape[0]
            N_ctx = context.shape[-2]
            C = context.shape[-1]

        N_text = x_text.shape[0]

        if N_text == B:
            # 1. 计算 EOS token 的位置 (此时 EOS 所在的索引刚好向后偏移了 N_ctx)
            eos_indx = text.argmax(dim=-1) + N_ctx

            # 2. 拼接: SOT (1) + Context (N_ctx) + 剩余文本。总长度恢复为原本的 77！
            x = torch.cat([x_text[:, 0:1], context, x_text[:, 1:]], dim=1)

            # 3. 直接加上原始的 position embedding (绝对不要用 interpolate)
            # 此时 x.shape[1] 就是原生的 context_length (如 77)
            ...
            # 4. 提取 EOS token 处的特征并投影
            x = x[torch.arange(x.shape[0]), eos_indx] @ self.text_projection

            return x

        # 情况 2: 原始 DenseCLIP 模式 (广播)
        # text 是 K 个类别, context 是 B 个图片上下文
        else:
            K = N_text
            N1 = x_text.shape[1] # text seq len
            N2 = N_ctx          # context seq len
