...
class DenseCLIP(BaseSegmentor):
    ...
    def __init__(self, backbone, text_encoder, context_decoder, decode_head, class_names,
                 ...
                 token_embed_dim=512, text_dim=1024, use_adapter=True, **args): # <--- 加上参数
        ...
        if getattr(self, 'with_adapter', False):
            def injection_func(l3_feat):
                ...
                if gt_semantic_seg is not None:
                    ...
                    # ====== 【替换下面的固定 kernel_size】 ======
                    # 计算当前图片的伪造区域占比
                    fake_ratio = clean_mask.sum() / (valid_mask.sum() + 1e-5)

                    # 动态设定边界宽度：面积越大（如 FS），边界提取核越小，防止梯度爆炸
                    if fake_ratio > 0.3:
                        kernel_size = 5   # FaceSwap 或 Deepfakes 等大面积替换
                    elif fake_ratio > 0.1:
                        kernel_size = 9   # 中等面积替换
                    else:
                        kernel_size = 15  # NeuralTextures 等微小替换，使用大核放大边缘特征
