...
class ForensicsAdapter(nn.Module):
    def __init__(self, vit_name='vit_tiny_patch16_224', clip_dim=1024):
        ...
        # 引入轻量级 ViT 作为旁路适配器
        # 定义你本地权重的绝对路径或相对路径
        local_weight_path = r"C:\Users\27298\.cache\torch\hub\checkpoints\Ti_16-i21k-300ep-lr_0.001-aug_none-wd_0.03-do_0.0-sd_0.0--imagenet2012-steps_20k-lr_0.03-res_224.npz"
        self.vit_model = timm.create_model(
            vit_name,
            pretrained=False,  # 设为 False，阻止联网
            num_classes=0,
            checkpoint_path=local_weight_path  # 强制从这里读取权重
        )
    ...
    def intra_contra_loss(self, adapter_feat, gt_semantic_seg):
        ...
        # 生成对角线为 1 的单位矩阵
        identity_mask = torch.eye(L, device=adapter_feat.device).unsqueeze(0)

        # 从正样本掩码中屏蔽自己 (对角线变为 0)
        pos_fake_mask = pos_fake_mask * (1 - identity_mask)
        pos_real_mask = pos_real_mask * (1 - identity_mask)
