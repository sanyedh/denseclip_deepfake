# 文件位置: segmentation/denseclip/denseclip.py
import torch
...
# 注意：根据你的截图，文件名是 untils.py (可能是作者笔误)，这里保持一致
from .untils import tokenize

@SEGMENTORS.register_module()
class DenseCLIP(BaseSegmentor):
    """DenseCLIP for Deepfake Detection with Dynamic Prompts."""
    ...
    def __init__(self,
        ...
        if pretrained is not None:
            # 避免重复加载 backbone 权重，如果 backbone 已经设置了 pretrained
            if backbone.get('pretrained') is None:
                backbone.pretrained = pretrained

            # 处理 Text Encoder 预训练权重
        ...
        # Learnable Contexts 初始化
        # 计算除去 prompt 文本后的剩余长度
    ...
    def after_extract_feat(self, x, text_prompts=None):
        ...
        if self.context_feature == 'attention':
            visual_context = torch.cat([global_feat.reshape(B, C, 1), visual_embeddings.reshape(B, C, H*W)], dim=2).permute(0, 2, 1) # [B, N_ctx, C]
        else:
            visual_context = None # Backbone 模式下可能不需要显式构造

        # 1. 动态处理文本 (Dynamic Text Encoding)
        # text_prompts 结构: [['Real desc', 'Fake desc'], ['Real desc', 'Fake desc'], ...]
        flat_prompts = []
        for p in text_prompts:
            flat_prompts.extend(p) # 展平为 [B * 2] 个字符串
        ...
        # 扩展 Contexts: [1, ctx_len, dim] -> [B*2, ctx_len, dim]
        ...
        # 2. 上下文交互 (Context Decoder)
        # text_embeddings: [B, K, C], visual_context: [B, N, C]
        ...
        # 使用 einsum 进行带 Batch 的矩阵乘法
        # b:batch, c:channel, h:height, w:width, k:class
        score_map = torch.einsum('bchw,bkc->bkhw', visual_embeddings, text)

        # 计算需要多少额外的通道来匹配neck期望的通道数
        # 从错误信息看，neck期望的是2050通道（2048+2），而不是2198（2048+150）
        # 所以我们不需要额外填充，score_map已经是2通道，加上原来的2048通道正好是2050通道
        padded_score_map = score_map

        # Concatenate the original feature map with the score maps
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg):
        ...
        losses = dict()
        # 如果开启 Text Head (辅助分支)，则把文本特征也传进去
        if self.text_head:
    ...
    def inference(self, img, img_meta, rescale):
        ...
        # 处理 TTA (Test Time Augmentation) 的 Flip
